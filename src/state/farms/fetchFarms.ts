  ...
  const data = await Promise.all(
    farmsConfig.map(async (farmConfig) => {
      ...
      if(farmConfig.isTokenOnly){
        ...
        if(farmConfig.token.symbol === QuoteToken.USDC && farmConfig.quoteToken.symbol === QuoteToken.USDC){
      ...
      const [info, totalAllocPoint, rnboPerBlock] = await multicall(masterchefABI, [
        {
      ...
      return {
        ...
        rnboPerBlock: new BigNumber(rnboPerBlock).toNumber(),
        ...
        farmWithdrawFees: new BigNumber(farmWithdrawFees).toNumber(),
      }
    }),
  )
  return data
